...
export class Redis implements Extension {
	...
	public constructor(configuration: Partial<Configuration>) {
		...
		// iovalkey is runtime-compatible with ioredis, but Redlock types its client
		// strictly to ioredis, so we bridge to the client type its constructor expects.
		this.redlock = new Redlock(
			[this.pub] as unknown as ConstructorParameters<typeof Redlock>[0],
			{
				retryCount: 0,
			},
		);
